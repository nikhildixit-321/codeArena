...
export default function HomeNavbar({ showSidebarTrigger = true }) {
  const { user } = useAuth();
    ...
    <div className="w-full h-16 bg-background/80 backdrop-blur-md border-b border-border flex items-center justify-between px-4 md:px-6 z-50">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="hidden md:flex items-center gap-4 text-sm font-medium">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-yellow-500/10 text-yellow-500 rounded-full border border-yellow-500/20" title="Coins">
            <Coins size={14} className="fill-yellow-500" />
            <span>120</span>
          </div>

          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-500/10 text-blue-500 rounded-full border border-blue-500/20" title="Energy">
